...
关闭的仅内存反馈菜单：显式开始、停止和清除分别独立，记录中只增加一个状态
圆点，不注册新的全局快捷键。现代任务栏入口使用随系统文字颜色变化的“自/A”
...
规范 `v` 词条。开发 DLL 另带一份项目自有、MPL-2.0 的小型电子硬件覆盖词表，
补充“丝印、焊盘、过孔、数据手册、使能、片选”等公开快照缺词，不修改固定
第三方词典，也不从私人输入隐式学习。
