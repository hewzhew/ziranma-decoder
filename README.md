...
TSF 组合与普通文档编辑能否处在同一可定位事件流里。查看层会按位置关系归并
连续插入、同向删除和同位置替换演变，但保留原始内存事件；插入后再删除仍是两组，
不会被一个模糊时间阈值藏掉。
...
停用即清空，且不写磁盘。奇数键暂态会先给“完整双拼前缀 + 最后一个声母”
保留最多三项，随后仍展示普通简拼；补齐韵母的偶数帧不受影响，也不会为掩盖
换字而延迟候选语义更新。
前台文本服务默认维护有界的滚动内存现场；输入法内的
...
普通候选首页可右键某一项，再从不抢焦点的底部操作条明确选择“提高 / 公共排序 /
撤销降低”。短候选窗会在当前屏幕工作区内临时扩宽，关闭操作条后恢复原位置与尺寸；
打开或取消不会写入偏好。没有声明 `InputScope` 的 Chromium 编辑器只允许这条显式
两步标注，不开放被动记录或自动学习；密码、PIN、私密和其他明确受限输入仍拒绝。
...
fail closed。V8 起的批次用进程内连续流、批次序号和事件序号把存储边界接回，主动许愿
...
首次非首选身份的脱敏双拼键长及分桶候选来源配对、候选窗首帧与完全显示耗时（含首次出现/后续更新及 `≥16/32/64/128 ms` 固定阈值长尾）、V9 慢按键阶段耗时（含由总耗时严格相减得到的其余阶段与主耗时阶段）、V10 提交后紧接退格线索、V11 六种可叠加个性化原因和 V12 公开共识校准来源，并按来源 schema 明确显示这些诊断能力覆盖了多少批次、当前格式是否已实际出现，再按 DLL 与候选数据版本区分新批次。它还会先连接 V8 及之后批次，再按失焦、
宿主结束和自适应停顿形成自然片段，将有锚点的许愿连接回现场，并把实际采用的自动
换序与重复改码分别列为证据和待确认线索；它不写模型、不联网，也不修改排序。旧
批次没有连续链或版本身份时会明确标为未记录，不根据文件时间推断。回顾会把时间上
最新的已标识 DLL 另列一段聚合，避免旧历史淹没刚换代后的变化；首次从非首选提交、
后来到过首选的身份还会按首次到达首选所需的选择次数分桶，并把到达后的首次回落
区分为同一连续流、同一运行身份的新连续流、不同运行身份或旧记录缺少边界，再比较
回落前后的 V11 精确原因；旧或混合格式只比较兼容标记，挡在前面的全局首选来源另行汇总。
首次非首选还会分别聚合目标来源、当时首选来源及其来源配对，并把 V11 精确个性化
原因与旧格式兼容标记隔离；V12 还把真正由核心/补充词典共识移到首位的整词单列，
帮助区分核心原始同码顺序、公开共识校准、组合路径和受保护候选。
该聚合还会跨连续批次配对奇数键暂态与下一键完成帧，只显示固定时延分桶、首选
变化和候选保留计数，不显示参与比较的候选文字；链不连续时不会猜测配对。
...
记录。选择“整理这条记录…”后，可以补充或修改类别与说明，并标为“待整理 / 处理中 /
已完成”和“普通 / 重要”；没有说明文字也能只保存整理状态。原始现场保持不可修改，
这些整理内容继续由当前 Windows 用户加密保存。旧说明自动按“待整理、普通”读取。
管理器顶部可以搜索说明和整理标签，并按状态或重要性筛选；比较只发生在当前窗口
内存中，不建立明文索引。顶部固定显示带数量的“许愿记录 / 回收站”双页签；两页
保持相同的筛选、列表和详情骨架，空状态也不会让整页布局消失。首次打开一页时读取
其整理说明，此后页签往返复用窗口内存缓存；每页只保留当前选中记录的已解密详情，
离开再返回不会重新读取，选择另一条时释放旧详情。“刷新”会重新核对磁盘清单和说明；
如果内容没有变化，就保留列表、选择和当前详情，不再制造一次整页重载，同时允许重试
先前暂时不可读的详情。再次核对时先比较加密说明文件的摘要，只解密新增或确实变化的
...
仍可切换。两键完整音节若已有两个未被忘记的个人文字，或一个持久文字与不同的
本轮会话文字，则无上下文的持久与会话精确置顶会主动 abstain，恢复稳定公开顺序；
匹配的左侧上下文仍可重排，忘记其中一个文字后剩余偏好重新生效。更长精确码和
只有一个个人文字的两键码保持原规则，因此低歧义整词学习不会被这一窄门削弱。
显式固定候选始终位于个人排序之前。
...
不删除正向证据，也不改公共词典。普通候选当前页只要出现带蓝点的个人记忆，页脚会
同时显示紧凑的“个人 · Ctrl+Del”提示；纯公共候选、其他页以及找字、换序和许愿视图
不显示这条提示，也不改变候选顺序。
