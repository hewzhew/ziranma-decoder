...
Decoder 创建时会把每个词的全码、简拼混合方式放入一棵 trie。查询时
先从实际按键生成所有“至多一次错误”的标准编码假设，再沿 trie 精确
查找；不再为每次输入遍历全部词条。
...
当前 trie 会在构建阶段展开每个词的 `2^n` 种简拼组合；它消除了查询时
遍历全部词条的问题，却仍不适合特别长的词。下一次算法扩展应把 trie
改成逐音节的一键/两键分支，换入更大的公开词典和有明确许可证的独立
公开语料，再做真实性更高的词外输入与延迟评测。
