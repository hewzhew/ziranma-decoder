...
首次非首选身份的脱敏双拼键长分布、候选窗首帧与完全显示耗时（含首次出现/后续更新及 `≥16/32/64/128 ms` 固定阈值长尾）、V9 慢按键阶段耗时（含由总耗时严格相减得到的其余阶段与主耗时阶段）、V10 提交后紧接退格线索、V11 六种可叠加个性化原因和 V12 公开共识校准来源，并按来源 schema 明确显示这些诊断能力覆盖了多少批次、当前格式是否已实际出现，再按 DLL 与候选数据版本区分新批次。它还会先连接 V8 及之后批次，再按失焦、
