...
路径与对齐状态、终点拼写路径/展开词条数、lattice 逻辑生成/完整物化/
最终保留的词边与未解析边、求解的 k-best 状态、缓存命中和路径组合数。
...
一次本机运行中单词 median 约 0.8 ms，短句约 24.0 ms，长句约
50.2 ms；这些数字不能跨机器比较。结构去重改用哈希索引后，短句/长句
...
匹配分离，所有词条只用标量索引参与缩减，将中位数降到上述结果。结果
校验和与不裁剪全路径 oracle 都不变。当前剩余成本主要是约 3 万次 trie
路径访问和 2.7–2.8 万条终点拼写路径。
...
减少 trie 路径展开和终点拼写路径；并把“完整词典覆盖”从绝对优先改成
