...
路径、精确证据预扫、子树剪枝与对齐状态、终点拼写路径/展开词条数、
lattice 逻辑生成/完整物化/最终保留的词边与未解析边、求解的 k-best
状态、缓存命中和路径组合数。
...
一次本机运行中单词 median 约 1.0 ms，短句约 19.0 ms，长句约
40.3 ms；这些数字不能跨机器比较。结构去重改用哈希索引后，短句/长句
...
`5,641 + 798,784`；当前剩余成本主要是 72,460 / 71,074 个终点词条
展开和约 80 万次已缓存对齐转移。
...
下一次算法扩展应直接处理测出来的两项问题：在保持三个错误层前沿独立
的前提下，为同一终点节点按词频上界停止展开不可能进入 Top-K 的词条；
并把“完整词典覆盖”从绝对优先改成可校准的拒识判据，避免陌生词被
简拼组合强行解释。随后再接入许可明确的独立公开句子语料训练上下文
模型。
