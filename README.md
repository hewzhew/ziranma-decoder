...
候选窗耗时、V9 慢按键阶段耗时（含由总耗时严格相减得到的其余阶段与主耗时阶段）、V10 提交后紧接退格线索和 V11 六种可叠加个性化原因，并按来源 schema 明确显示这些诊断能力覆盖了多少批次、当前格式是否已实际出现，再按 DLL 与候选数据版本区分新批次。它还会先连接 V8 及之后批次，再按失焦、
...
原因与旧格式兼容标记隔离，帮助区分同层冷启动词序、组合路径和受保护候选。
