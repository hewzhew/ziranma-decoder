...
`public-compose-evaluate` 从固定 UD test 依源序选择 64 个相邻双词、
2～6 字、Rime 整词可覆盖的自然短语。尾部简写把 430 个完整码字母降到
343 个，节省 20.2%；主榜 Top-1/3/5/10 为
`21/32/43/45`，相邻颠倒恢复栏为 `22/34/41/47`。在主榜已经返回的
Top-10 内只过滤成正确字数，几乎不提升结果；这说明主要困难是同长度
短语排序，而不是一字/多字混淆。
