...
路径与对齐状态、lattice 生成/保留的词边与未解析边、求解的 k-best
状态、缓存命中和路径组合数。
...
一次本机运行中单词 median 约 3.6 ms，短句约 98.7 ms，长句约
191.0 ms；这些数字不能跨机器比较。结构去重改用哈希索引后，短句/长句
中位数先从约 `1.21 s / 2.56 s` 降到 `128.9 / 251.1 ms`；随后提前
精确缩减把两条短句的 lattice 边从 94,360 条保留到 556 条，长句从
92,839 条保留到 583 条，并进一步降到上述结果。排名器实际接收的转移
和原来相同（743 / 795），路径组合仍为 5,720 / 7,185。结果校验和与
不裁剪全路径 oracle 都不变。当前剩余工作主要是约 85 万个对齐状态和
9.4 万条边的生成成本，而不是已经压紧的排序阶段。
...
下一次算法扩展应直接处理测出来的两项问题：在 trie 扫描中更早避免
无效对齐和注定会被丢弃的候选边，而不只是生成后缩减；并把“完整词典
