...
一次本机运行中单词 median 约 2.0 ms，短句约 39.3 ms，长句约
75.5 ms；这些数字不能跨机器比较。结构去重改用哈希索引后，短句/长句
...
小分配，得到上述结果。排名器实际接收的转移仍为 743 / 795，路径组合
仍为 5,720 / 7,185。结果校验和与不裁剪全路径 oracle 都不变。当前
剩余成本主要是约 85 万次对齐检查和约 9.4 万条轻量终点记录。
...
下一次算法扩展应直接处理测出来的两项问题：用可证明的前缀上界或等价
状态复用减少 trie 扫描本身的对齐检查和轻量终点记录；并把“完整词典
覆盖”从绝对优先改造成可校准的拒识判据，避免陌生词被简拼组合强行
解释。随后再接入许可明确的独立公开句子语料训练上下文模型。
