...
实例中置顶；空格首选、换序与 Tab 候选不学习，
停用即清空，且不写磁盘。反馈默认关闭，显式开始、停止和清除分别独立；记录
期间的“向猫猫许愿”会冻结此前最多 30 秒的输入法自身事件，使用 Windows 当前
用户 DPAPI 加密后写入本地；保存失败不停止反馈，也不影响输入和候选窗。
Windows 11 现代输入指示器并不可靠展示第三方语言栏项目，因此语言栏菜单只作
可选入口；`wishpad` 面板/托盘和 `wish-ime.cmd` 通过 TSF 当前用户全局 compartment
发送无正文整数命令。输入码、候选、留言、路径和快照都不跨进程，不注册全局
快捷键，也不连接网络。真实 Codex 与密码控件仍需逐步验收；任何本机注册仍
需要用户明确确认。
...
不带参数的 `wish-ime.cmd` 会打开单实例许愿面板；面板关闭后继续在托盘待命，
再次运行快捷方式或左键单击托盘图标会直接唤回同一面板。开始、保存近 30 秒、
补充说明和停止都在面板中完成；托盘右键菜单只保留打开、清除和退出。保存现场后，
面板中的“补一句说明…”会打开独立小窗，可选择
候选、排序、显示、延迟、输入模式、兼容性或其他并写下说明；说明与打开窗口时
选定的那条许愿绑定，由当前 Windows 用户加密保存，不联网，也不会覆盖已有说明。
成功执行 `update-ime.cmd` 后也会打开或唤回该面板，不设置开机启动或全局快捷键。
在输入法内也可以精确输入 `xuy` 后按 `Tab`：反馈尚未开始时显示“开始反馈 / 暂不保存”，
记录中显示“向猫猫许愿 / 近 30 秒”，停止后显示“清除反馈 / 已停止”。空格
或数字 1 明确确认，退格、Esc 或再次 Tab 返回；`xuy + 空格` 仍按普通候选上屏，
所有非 `xuy` 的 Tab 路径保持不变。
...
当前字母码；中文模式下 `,` / `.` / `;` 与 `Shift+;`、`Shift+1`、`Shift+9/0`、
`Shift+/` 分别输入常用全角标点，有组合时先确认首选再追加标点。其他可打印键
会先结束预编辑再交还宿主；带 Shift 的数字不会误选候选。交互候选会先列任意长度的完整精确码，
再补自由简写结果；`ju` / `qu` / `xu` / `yu` 可使用通常的 `u` 写法匹配自然码
规范 `v` 词条。开发 DLL 另带两份项目自有、MPL-2.0 的小型覆盖词表：技术层
补充“丝印、焊盘、过孔、数据手册、使能、片选”等公开快照缺词，会话层补充
