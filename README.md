...
`ITfCandidateListUIElement` 与不抢焦点的七项弹窗。首项使用略亮圆角底和
左侧柔和天蓝色短条；首选汉字为近白半粗体，普通汉字、序号和分页依次降低
明度与字号，避免七项同亮。数字键明确选过的普通同码候选会在当前文本服务
实例中置顶；空格首选、换序与 Tab 候选不学习，
停用即清空，且不写磁盘。源码还为 TSF 原生“中/英”输入模式入口加入了默认
关闭的反馈菜单：显式开始、停止和清除分别独立，记录中只增加一个状态圆点，
不注册新的全局快捷键。记录期间的“向猫猫许愿”会冻结此前最多 30 秒的输入法
自身事件，使用 Windows 当前用户 DPAPI 加密后写入本地；保存失败不停止反馈，
也不影响输入和候选窗。现代任务栏入口使用随系统文字颜色变化的“自/A”
单色图标，经典语言栏继续显示文字并作为兼容回退。真实 Codex 与密码控件仍需
逐步验收；任何本机注册仍需要用户明确确认。
...
```powershell
cargo build --release --bin wishctl
.\wish-ime.cmd status
.\wish-ime.cmd list
...
再追加标点，Shift 组合仍由宿主处理。交互候选会先列任意长度的完整精确码，
