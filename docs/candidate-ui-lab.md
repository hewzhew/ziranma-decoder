...
当前仍没有可运行的候选窗实验室，也没有 A/B、圈选、拖动参数或本地批注 UI；
共享 painter 只是让以后原生实验室能复用生产真值，不等于这些交互已经完成。下一步
可以先为 `CandidateScene` 增加语义区域 hit-testing，固定重叠区域的命中优先级；
也可以建立只装载公开合成候选的原生 Windows 实验室骨架。两条路径都必须继续调用
同一个 scene 与 GDI painter，不能用网页近似预览冒充真实效果。
