...
内容。浏览器只在内存中保留一份已验证批次；所选条目可通过独立的“打开只读重放”
按钮重放到共享 GDI painter。重放窗口按该条目的公开场景、横竖布局、固定 DPI 和完整
视觉规格重新计算客户区，并直接调用与生产候选窗相同的 scene、字体和 painter；圈选框
也用保存的物理像素矩形覆盖显示。翻阅前后条目时，已打开的重放窗口同步更新尺寸与
内容；若圈选越出重建后的固定公开场景，文字仍可浏览，但重放会明确拒绝。重放不把
历史规格写入 A/B 草案，不改变主预览，也不生成新的批注或文件。
