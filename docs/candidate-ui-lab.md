...
横/竖布局、固定 DPI、裁剪后的圈选矩形、稳定语义命中与候选索引，以及完整
`CandidateVisualSpec` 的 SHA-256；不保存公开候选文字。批注会规范换行、拒绝不支持
的控制字符，并限制为 240 个 Unicode 字符；一次运行最多暂存 64 条。保存失败时输入
窗与草稿仍保留，取消或关闭输入窗则不加入记录；主实验室关闭时也会显式销毁子窗并
恢复窗口状态。标题只显示本次运行的批注数量。
...
场景、两种布局与四种固定 DPI；不满足行高、首选底高或最小可读宽度关系的中间值会被
拒绝。下一步是在同一模型上增加并排、同屏的 A/B 预览，再考虑颜色与彼此耦合更强的
细项，而不是另造一套调参数据模型。
