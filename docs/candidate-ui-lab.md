...
当前单条格式为 `candidate-ui-lab-annotation-v2`，批次格式为
`candidate-ui-lab-annotation-batch-v2`。批次在保持批注原始会话顺序的同时，额外输出
按 `(variant, visual_spec_sha256)` 确定性排序的 `spec_groups`；每组只含稳定变体 ID、
完整 spec 哈希和批注数量，不复制批注文字或候选正文。这样未来的本地批注浏览器可以
按同一份视觉草案归并 A/B 反馈，而不用从自由文本或坐标猜测方案身份。v1 与 v2 是明确
不同的 schema；当前工具没有导入器，也不会把旧 v1 文件静默解释成带变体分组的 v2。
...
第一组只开放外边距、行高、文字与序号间距、横竖宽度、圆角、首选底色高度与蓝条宽度、
候选和序号字号等 14 个几何 token。每次切换或修改都会重新构造相同 scene、字体和
painter，批注因此绑定当前实际草案的 spec 哈希，而不是只记一个“A/B”标签。基线从不
被修改，草案也不会回写 Rust 源码或生产 DLL。所有 token 的上下界均穷举经过三种公开
场景、两种布局与四种固定 DPI，而且同时构造 A 与 B 两套 scene；不满足行高、首选底高
或最小可读宽度关系的中间值会被拒绝。下一步是在同一模型上增加颜色与彼此耦合更强的
细项；未来批注浏览器应直接消费 v2 的稳定变体与 spec 分组，而不是另造一套调参数据
模型。
