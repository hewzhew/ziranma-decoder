...
这些批注目前仍**仅在内存中**，关闭程序便消失，不会写文件或联网。规范 JSON 序列化
已经由测试固定，但尚未接入导出入口，因此不能把内存计数误解成已经保存。下一步应先
实现显式、不覆盖、带容量上限且原子写入 Git 已忽略目录的本地导出，再做参数面板、
A/B 冻结和拖动 token；实验室也尚未加入日用用户工具槽。
