...
选中底、选中竖条、页脚和分隔线矩形；96/144 DPI 的精确矩形测试以及生产
96/120/144/192 DPI 布局联结测试固定了这条边界。此次提取没有修改视觉 token、
候选数量、截断或排序行为。

第二阶段仍在进行中。候选数字与正文的字体度量、基线对齐、文字裁切和具体 GDI
绘制原语仍由 `tsf_alpha.rs` 执行；这些部分尚未形成可由独立实验室直接调用的
`paint_candidate_scene`。因此当前没有可运行的候选窗实验室，也没有 A/B、圈选、
拖动参数或本地批注功能。下一步应先把文字/序号/页码的语义区域接入共享 scene，
再提取同一真实 GDI painter，不能用网页近似预览提前冒充完成。
