...
正文的垂直对齐不再由 painter 重算。`candidate.rank`、`candidate.text`、
`candidate.personal-mark` 和 `notice.icon` 也已有稳定语义名。96/144 DPI 的精确
矩形测试以及生产 96/120/144/192 DPI 布局联结测试固定了这条边界。此次提取没有
修改视觉 token、候选数量、截断或排序行为。

第二阶段仍在进行中。按真实文字测量结果执行的省略、单项操作提示的动态分栏、
页脚内部的模式/页码区域和具体 GDI 绘制原语仍由 `tsf_alpha.rs` 执行；这些部分尚未
形成可由独立实验室直接调用的 `paint_candidate_scene`。因此当前没有可运行的候选
窗实验室，也没有 A/B、圈选、拖动参数或本地批注功能。下一步应先完成页脚与操作
提示的语义区域，再提取同一真实 GDI painter，不能用网页近似预览提前冒充完成。
