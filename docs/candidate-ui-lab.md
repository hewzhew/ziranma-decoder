...
实验室现已先建立不依赖面板的 A/B 草案边界：`A` 在不可修改的生产默认基线和仅存于
实验室内存的草案之间切换，`T` 按固定顺序轮换参数，`-` 与 `+` 有界调小/调大，`R`
把草案精确恢复为基线。第一组只开放外边距、行高、文字与序号间距、横竖宽度、圆角、
首选底色高度与蓝条宽度、候选和序号字号等 14 个几何 token；颜色和彼此耦合更强的
细项留给可显示范围及约束的原生参数面板。每次切换或修改都会重新构造相同 scene、字体
和 painter，批注因此绑定当前实际草案的 spec 哈希，而不是只记一个“A/B”标签。基线
从不被修改，草案也不会回写 Rust 源码或生产 DLL。所有 token 的上下界均穷举经过三种
公开场景、两种布局与四种固定 DPI；不满足行高、首选底高或最小可读宽度关系的中间值
会被拒绝。下一步是在这条已测试的状态机上增加可见参数面板和同屏 A/B，而不是另造
一套调参数据模型。
