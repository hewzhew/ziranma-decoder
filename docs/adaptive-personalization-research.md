...
- 只连接同一合格输入范围中、由 TSF 文档范围确认相邻的连续提交；每次上屏前克隆
  范围，下一组合开始时核对同一 Context、端点相接且锚点文字未变。光标移动、选区
  替换、外部编辑、Context 变化、标点、普通首选提交、取消、焦点切换、输入模式
  切换和宿主快捷键都会断链；宿主拒绝范围能力时明确回退到连续键盘语义；
...
第四层 `--personal-left-context-comparison --compact` 不再以整段连续窗口
作为学习单位，而是逐个合格提交复现 TSF 的精确左上下文身份。它在同一
规范码候选池上并列公共排序、冻结/因果精确码控制路线，以及冻结/因果
`上一提交文字 + 当前真实码 + 当前选择文字` 路线。当前提交始终先评分后
学习；后续确定位置编辑若与任一关系跨度重叠，会撤销相关精确码和上下文
证据。报告分别给出上下文相对各自精确码控制路线的 Top-1 得失和名次变化，
并统计目标、竞争证据与历史重复身份。只有相对精确码控制仍有独立净收益，
才值得讨论左上下文持久化。旧胶囊没有可靠的“被明确越过候选”字段，因此
该实验只学习正选择，并在报告中公开 `selection_rejections=unavailable`；
不会用重建候选列表猜造反证。

该报告现进一步拆出精确码与左上下文的实际首位移动、目标/竞争证据，以及一次/
重复赢家支持。一次按时间留出的本地敏感性对照显示：累计完整历史会保留一项由
重复竞争证据造成的首选损失，简单重复门槛无效；只用最近一个历史会话时损失消失
且已有首选收益未减少，但最近两个或三个会话又出现损失。这个单一留出只足以否定
无期限原始累计和固定重复门槛，不足以选择“一会话过期”。

这项独立路线现已实现为 `--personal-tiered-code-comparison --compact`：操作者必须
分别点名旧历史、近期历史与未来评测；近期有同码池内证据时由近期层接管，否则
旧层只能在公共首选之后回退。它同时报告未保护的旧层原本会造成哪些得失。当前
单一末会话留出中，完整冻结路线为 54 次首选收益、7 次损失；保护式分层路线为
50 次收益、4 次损失，相对完整路线修复 3 次却新增 4 次退化。近期接管的 4 次损失
和旧层回退的 3 次损失都已有重复支持，因此固定重复门槛仍不能分离得失。这条规则
未通过安全门，不接入 TSF；工具保留用于更多独立会话，而容量与权重继续是显式
研究参数。
