...
提交、取消和上下文资格都会经过输入法自身记录。没有明确许愿时它不写文件，
应用退出即丢失。“许愿”只在这条边界上冻结短时现场；“应愿”再负责本地查看、
整理和可审阅导出，而不是另建全局键盘钩子或读取宿主周围正文。
...
1. 前台文本服务启动时建立有界滚动内存现场；它不写文件，宿主退出即消失；
...
- Alpha 维护有界内存环，在用户明确许愿时冻结、DPAPI 保护并原子保存一个快照；
...
这些参考只决定入口层级：管理器仍是有界本地工具，不加入遥测、网络提交、
隐式捕获、自动启动或驻留在 TSF 宿主里的复杂界面。
...
- 滚动现场只存在于前台文本服务的有界内存中；没有明确许愿就不写文件；
