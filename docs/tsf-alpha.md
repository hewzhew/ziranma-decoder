...
- 事件默认只存在于调用进程内存；只有用户在记录中明确选择“向猫猫许愿”时，
  才复制此前最多 30 秒并使用当前用户 DPAPI 写入稳定的本地许愿目录；不建立
  IPC，也不连接网络；
...
留下脱敏失败状态，输入法继续工作。留言通过独立 `wishctl annotate` 明确提供，
和快照分别加密但绑定同一不可变 ID；完整的前台 `wishpad`、逐项预览和可选尾迹
仍是后续工作，见[“向猫猫许愿”：本地反馈与现场快照](wish-feedback.md)。
