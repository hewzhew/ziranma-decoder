...
- 双拼奇数键暂态不再完全交给自由混合简拼：候选首先尝试“此前音节全部完整、
  ...
  不通过延迟绘制掩盖候选变化；
- 普通 `Tab` 已接入公开单字笔画辅助：当前组合必须恰为一个有公开精确单字池的
  两键完整双拼码；进入后冻结该池，交互键使用自然码声母位置
  `h/u/p/n/v`（横、竖、撇、捺、折），内部映射为公开数据的规范
  `h/s/p/n/z` 笔画码后稳定过滤；旧的 `s/z` 输入暂作兼容，
  Backspace 撤回一画、空前缀时返回普通候选，Esc 直接返回。候选窗显示“找字”，
  ...
  对照自动纠错，不与找字共享产品语义；
- 四、六、八键的完整二至四字码现在可以在每段均有公开精确单字池时逐段找字。
  候选窗先显示“找第 1 字”，选定后用“雀 → 第 2 字”“雀魂 → 第 3 字”一类动态
  路径标明已选文字；中间字符只保留在当前组合内，不写文档、不记为独立选择。
  选完最后一字后一次提交完整词，并把这次明确组装作为可立即 Backspace 撤销的
  个人整词证据。空笔画时 Backspace 逐字退回上一段，Esc 返回原始码串；标点不会
  误提交半成品。五字以上、简拼分段和自动组词仍不在当前范围；
...
当前页后已有候选/仍可继续加载/已到底、Tab 逐字组词位置/笔画前缀和可选自动换序凭据；
