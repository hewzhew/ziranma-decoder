...
- 公开快照仍随类工厂固定；显式私人别名则来自所有不可变 DLL 构建共享的
...
- 菜单只提供当前状态允许的动作：未开始时为“开始反馈（仅内存）”，记录中为
  ...
  后才能开始下一轮；记录中和停止后还会以禁用菜单项只读显示双拼半音节间隔
  的分桶与计数；
...
成功路径只显示结果、DLL 摘要、当前用户启用状态和默认输入法
边界；底层检查的详细输出仅在失败时展开。Windows PowerShell 5.1 对原生程序
