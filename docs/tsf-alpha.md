...
- 连续两个至四个由公开候选源验证的“两键完整码 + 单字”明确选择会组成精确个人
  短语。第二字先产生可立即召回的二字待确认事务；第三、第四字到来时先确认上一层
  前缀，再建立更长事务。紧接空组合 Backspace 只撤销最新事务并恢复加入该字前的
  前缀链，否则跨过下一按键、焦点离开或停用边界后，该长度一次就进入现有当前用户
  DPAPI 个人排序。第四字后的第五个合格单字确认四字事务并重新起链，不学习五字串
  或滑动重叠四字串。各长度短语、组成单字及其 suppression 互相保持精确身份。
  第一页空格首选、标点、简拼、纠错、Tab、换序、原码和宿主快捷键会拒绝或断开
  组合。当前尚未用文档范围锚点识别同一 Context 内的鼠标光标移动；
