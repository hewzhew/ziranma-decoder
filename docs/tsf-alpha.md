...
- 普通 `Tab` 已接入公开单字音形辅助：当前组合先拆成至多四个音码槽；完整
  ...
  选择仍通过同一 TSF 提交路径，但不写个人排序。约 1.2 MiB 的固定公开笔画表只在
...
- 二至四个音码槽可以在每槽均有公开精确单字池时逐段找字；只有最后一槽允许
  ...
  组合内，不写文档、不记为独立选择。
  选完最后一字后一次提交完整词，并把这次明确组装作为可立即 Backspace 撤销的
  个人整词证据；含尾声母的路径在完整两键身份随候选传到学习层以前只提交、不学
  短码。空形码时 Backspace 逐字退回上一段，Esc 返回原始码串；标点不会误提交
