...
- 组合码精确为 `xuy` 时，普通 Tab 直接按最近完成的输入片段保存一次许愿并显示
  ...
  或后续搜狗式笔画/部件找字协议；
- 事件默认只存在于调用进程内存；只有用户明确许愿时，才按输入片段或 30 秒兜底
  复制历史，并使用当前用户 DPAPI 写入稳定的本地许愿目录。结构化包区分参考
  上下文、重点片段与 `xuy` 触发入口；控制通道不携带输入码、候选、路径或快照，
  也不连接网络；
...
片段可以判断可见恢复候选是否被采用；影子命中也不再丢失。只有用户明确许愿时，
这些私人凭据才随 V5 快照由当前用户 DPAPI 加密写入；它们不自动写模型，也不实时
改变持久模型。当前进程内另有一个有界分层校准器：只有可见恢复候选随后被同一码
