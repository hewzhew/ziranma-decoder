...
- 更深同码双字的精确短词层使用另一个固定根
  ...
  release 状态路径已重复通过，但它不测候选窗绘制、实际宿主或换包延迟，这些门通过
  前仍保持关闭；
...
- 在线个人排序来自共享的
  ...
  撤回事务并恢复此前的会话偏好；若紧接着重打完全相同的码，刚撤销文字只在这一次
  候选窗中从未保护的首位暂退到第二位，其他码、Tab/换序视图、焦点变化或完成一次
  选择都会清除此内存提示。它不永久遗忘，也不越过显式别名等保护前缀。下一个按键、焦点离开或停用才确认。标点确认的
  ...
  重复达到同等支持后仍可切换。唯一例外是两键完整音节已有至少两个未 suppression
  的个人文字，或一个持久文字与不同的本轮会话文字：无左侧上下文时，持久与会话
  精确路径都停止全局置顶，保留公开顺序；
  精确匹配的进程内左侧上下文仍可移动现有候选。忘记其中一个竞争文字会重新放开
  剩余偏好；更长精确码与只有一个个人文字的两键码不受影响。独立的
  `personal-suppression` 当前用户加密动作
  ...
  遗忘与恢复动作，保存失败、密码框与未知输入范围均不产生成功事件。反馈 V16
  再以无文字、无位置事件记录合格个人短语组件的文档邻接或断链判定；
  已验证的公开完整整词证据可继承到同词的规范尾简：至少保留首个完整音节，
  只缩短一个非空尾部。目标文字已在普通短码候选池时按原位提升；池外公开精确
  整词一次确认后也只能进入固定前缀与第一个普通候选之后的发现位，不抢普通首选。
  短码精确证据优先，别名、自由分句和任意前缀不参与；短码抑制只关闭该视图。
  仅能逐字 Tab 验证的二至四字个人词采用更慢的发现门：完整码至少确认两次、每个
  ...
  三字和四字完整词覆盖每一种“完整前缀 + 连续尾简”，多个来源先过滤公开验证与
  suppression，再按有界个人支持选择；一个短码的忘记不影响兄弟尾简或来源完整码。
  公开完整码核验另有 128 项仅内存 LRU，并把补充词层版本纳入身份；它缓存正反
  结果但不落盘、不进入反馈，版本变化自然重新核验；
...
- 文档焦点离开、前台输入法切换和文本服务停用都会为旧 Context 排入异步取消。
  ...
  新组合，迟到的旧任务不会删除新输入；
- 失焦清理已被 TSF 接受、但异步编辑会话尚未执行时，逻辑码已经清空而旧文档
  组合仍短暂占用槽位。若焦点此时恢复并收到一个普通中文模式字母，测试回调会
  把真实按键送到 `OnKeyDown`，后者先尝试同步结束那一个确切旧组合，再建立新
  组合。旧 Context 拒绝同步编辑时返回 `FALSE`，把原按键完整交还宿主，不能为
  了“看似捕获”而吞键；已经排队的旧异步任务晚到后按组合身份变成无副作用的
  no-op，不隐藏新候选窗、清空新文档跟踪或删除新首键；
...
- 英文模式不接管普通按键或宿主快捷键。中文模式下的 `Shift + 字母` 与
  ...
  英文字母落入未完成的拼音组合；
- 模式目前属于每个宿主中的文本服务实例，不修改 Windows 默认输入法；内部状态
  已发布到标准 TSF 开关与转换 compartment，但 Windows 11 现代输入指示器是否
  展示第三方图标仍由系统决定。
...
- 五键或七键若恰好多按一枚与相邻键互为 QWERTY 邻键的字母，且核心与补充两套
  公开词典独立选择同一个两字或三字完整码，宿主把该整词插入第二项。该恢复不越过
  显式别名、项目词或其他保护前缀，也不把“完整词 + 下一音节首字母”武断改成首选；
- V20 诊断只在上述恢复确实出现在普通候选首页时，记录该组合完整的逐字母送达间隔、
  公开共识目标码、删除位置与可见名次。非连续追加、退格、缺少前序或候选未显示都不
  生成事件；它只为后续核对“快按是否真能区分多按”保存证据，不参与本轮候选排序；
- V21 诊断在普通首页的两键完整码上，把“持久同码竞争 / 会话同码竞争 / 明确未触发”
  附到全局首选来源。它解释无上下文精确个性化为何保留公开顺序，不记录竞争文字，
  不替代真实个人重排原因，也不改变候选；旧批次与缺少首页来源的现场保持未知；
...
- 普通首页候选还提供不抢焦点的显式鼠标标注：右键某一项后，候选窗在屏幕工作区
  内按需扩宽并在底部显示目标预览及“提高 / 公共排序 / 撤销降低”，取消后恢复
  原来的精确位置和尺寸。单纯打开或取消不写个人偏好，必须再明确点击一次动作；
  没有声明 `InputScope` 的 Chromium 编辑器只开放这条两步主动入口，仍不得产生
  被动反馈或自动学习。密码、PIN、私密、键盘禁用、空上下文和明确受限输入继续
  fail closed；右键按下与弹窗始终由候选窗消费，不把鼠标消息误送给宿主编辑器；
- 数字键明确选择的普通候选会进入当前文本服务实例的有界内存记忆；再次输入
  完全相同的码串时，该候选稳定移到首位。公开完整整词还可按上一节的相同约束
  影响规范尾简；会话记忆只提升已有普通候选，持久证据才可按受保护发现门补入池外
  公开整词，二者都不向任意前缀传播。空格确认首项、Enter 原码提交、换序
  候选和 Tab 找字都不产生记忆。最多保留 128 组，文本服务停用即清空，不序列化、
  不写文件，也不改变公开候选包；
- 连续两个至四个由公开候选源验证的“两键完整码 + 单字”明确选择会组成精确个人
  短语。每次合格提交在 `EndComposition` 前克隆组合范围，并把双端 gravity 设为
  backward；下一组合开始时，在同一同步编辑会话内验证 Context 身份、新范围起点与
  旧范围终点相接，并复核旧锚点文字未变。光标移动、选区替换、锚点文字被外部编辑
  或 Context 改变会断链；宿主不支持范围克隆、gravity、比较或读取时则明确退回原有
  连续按键语义，不让范围能力失败影响上屏。第二字先产生可立即召回的二字待确认事务；
  第三、第四字到来时先确认上一层前缀，再建立更长事务。紧接空组合 Backspace 只
  撤销最新事务，并同时恢复加入该字前的组件链和文档锚点；否则跨过下一按键、焦点
  离开或停用边界后，该长度一次就进入现有当前用户 DPAPI 个人排序。第四字后的第五
  个合格单字确认四字事务并重新起链，不学习五字串或滑动重叠四字串。各长度短语、
  组成单字及其 suppression 互相保持精确身份。第一页空格首选、标点、简拼、纠错、
  Tab、换序、原码、宿主快捷键、焦点/模式/停用边界会拒绝或断开组合，并清除范围
  锚点。反馈 V16 只在合格组件的候选提交事件之后记录邻接类别与前后 0–4 组件数，
  不含文字、位置或 Context 地址；
...
- 自绘 Alpha 面板每页最多显示 6 个候选。首项使用轻微提亮、上下留白的圆角底
  ...
  短候选在可用空间内按自然宽度横排；长候选共享一个最多 760 逻辑像素、且不超过
  工作区宽度 80% 的预算，横排首选保留最多约 16 个汉字，普通项才压缩。屏幕无法
  容纳各项最小宽度，或首选本身超过横排预算时，确定性回落为竖排；竖窗按最长可见
  候选扩到最多 660 逻辑像素，使 32 字显示上限仍有完整文字预算。必须截断时使用
  单个 Unicode 省略号，超过 32 字的来源也会保留这个标记而不是静默消失。
  外框内边距由 8 收紧为 5 逻辑像素，
...
- 服务激活与内部中英切换同步线程级标准 TSF compartment：中文令
  ...
  输入指示器一定呈现第三方图标；
- 因此另有一个可选、单实例的原生 `mode-indicator` 通知区伴侣。文本服务在激活、
  获得输入焦点和中英切换时，向固定的当前用户 TSF 全局 compartment 只发布
  `Chinese=1` 或 `English=2`；伴侣订阅这个整数并显示互不遮挡的“小猫 + 中”或
  “小猫 + A”。它不读取输入码、候选、正文、窗口标题或文档身份，Explorer 重启后
  会重新添加图标，右键菜单只显示当前状态与退出。当前阶段不自动启动、不写启动项；
...
- `candidatectl popup-render-preflight --repetitions <1..20>` 是另一条默认不运行、
  ...
  `BitBlt`、`EndPaint`、隐藏和销毁路径；固定覆盖首次创建、原位更新、第二页重绘、
  长候选竖排扩宽与 96/120/144/192 DPI。报告只有阶段耗时与 DPI，不含候选正文；
  窗口会在命令运行时短暂显示，但不注册、安装、换代、读取候选槽或反馈记录。单独的
  `DwmFlush` 只作为桌面合成队列同步边界，不能解释为显示器已经扫描或真人已经看见，
  也不代表具体编辑器宿主的调度。
- `candidatectl exact-phrase-popup-preflight` 是更窄的三包可见门：先用真实合成 Context
  发现并提交已认证三字层的第一页稳定探针；在创建 Context 或窗口前，共享三包门已
  对全部三字目标聚合核对第 1–6 项、既有完整词前缀、缺失、重复、跨页和结果上限，
  探针选择先覆盖每个非空目标位次桶，剩余名额才作分散取样；请求不足会在创建窗口前
  失败。随后再把同一探针页面和
  `PublicConsensusExact` 标记交给上述生产 popup 路径。它限制为 1–4 个页面、每页
  1–5 次重复并覆盖四档 DPI，运行时会短暂显示最多 80 个非激活窗口；因此默认不运行，
  不能被状态检查或构建命令隐式触发。两段是串行组件验证，不是已安装编辑器的一次
  回调，也不会写三字运行时凭据或启用状态。
