...
- 语言栏项目在文本服务激活时添加、停用时移除。Windows 11 现代输入指示器可能
  ...
  整理已经保存的本地许愿；
- 组合码精确为 `xuy` 时，普通 Tab 直接按最近完成的输入片段保存一次许愿并显示
  短暂回执；近 30 秒兜底仍可从语言栏或管理命令触发。没有滚动现场的开发测试
...
冷启动档位与实际采用档位都进入同一 V5 凭据；双音节组合凭据暂不进入这个单音节
...
许愿 V5 保留每条事件相对点击时刻的毫秒数，不把宿主单调时钟绝对值写入包，
并标记参考前文、重点片段、触发入口、截取方式、候选来源和可选自动换序凭据；
V1/V2/V3/V4 继续兼容读取。
