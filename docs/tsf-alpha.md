...
- `a`～`z`、Backspace、Esc、Space、Enter、常用中文标点与数字键的路由和
  ...
  `Shift+1`、`Shift+9/0` 与 `Shift+/` 分别上屏 `，` / `。` / `；` / `：` / `！` /
...
- Windows 11 现代输入指示器不保证为纯 `BTN_MENU` 项目调用 `InitMenu`。输入
  ...
  有效动作才会改变内存会话；
- 菜单只提供当前状态允许的动作：未开始时为“开始反馈（暂不保存）”，记录中为
  “停止反馈”，停止后为“清除本轮”。停止后的会话不能被再次开始覆盖；先清除
  后才能开始下一轮；记录中和停止后还会以禁用菜单项只读显示双拼半音节间隔
  的分桶与计数；
- 语言栏项目在文本服务激活时添加、停用时移除。Windows 11 现代输入指示器可能
  不展示它；此时单实例 `wishpad` 面板或 `wish-ime.cmd` 通过当前用户 TSF 全局
  compartment 发送开始、保存、停止和清除四种无正文整数命令。快捷方式与托盘
  左键直接唤回面板，托盘右键只保留打开、清除和退出；
- 组合码精确为 `xuy` 时，普通 Tab 会显示一个状态相关的反馈确认项；空格或
  数字 1 才执行，退格、Esc 或再次 Tab 返回，继续输入字母也不会吞键。
  `xuy + 空格` 和所有非 `xuy` 的 Tab 路径不变，因此该助记入口不占用普通候选
  或后续搜狗式笔画/部件找字协议；
- 事件默认只存在于调用进程内存；只有用户在记录中明确选择“向猫猫许愿”时，
  才复制此前最多 30 秒并使用当前用户 DPAPI 写入稳定的本地许愿目录；控制通道
  不携带输入码、候选、路径或快照，也不连接网络；
...
许愿 V1 保留每条事件相对点击时刻的毫秒数，不把宿主单调时钟绝对值写入包。
...
`wishpad` 说明小窗明确提供，和快照分别加密但绑定同一不可变 ID；小窗只处理
打开时选定的最近一条许愿，逐项预览和可选尾迹仍是后续工作，见
