...
拼音和禁用闭环。另有显式的当前用户启用和禁用命令；启用只让配置可供用户
选择，不会主动激活。候选窗口代码已进入下一构建，但尚未经过真实宿主验证。
...
- 候选数据一次解码至固定 Top-10，不再为了每个名次重复运行解码器；
- 实现标准 `ITfCandidateListUIElement`，普通宿主与 UIless TSF 宿主读取同一
  候选、页位置和当前选择；
- 普通宿主使用最多五项的不抢焦点弹窗，通过 `ITfContextView::GetTextExt`
  跟随真实组合范围，而不是读取屏幕或猜光标；
- `-` / `+` 与 PageUp / PageDown 在两页间切换，数字键按当前页选择；
- 提交、取消、宿主终止组合、焦点离开和文本服务停用都会结束 UI 元素并隐藏
  弹窗；
- 弹窗按宿主 DPI 缩放并限制在当前显示器工作区；暗色、字体、辅助功能和
  候选窗口消失/重建仍需真实宿主验证；
- Shift+Tab 显式换序；
- Tab 单字笔画辅助；
