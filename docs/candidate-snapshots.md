...
固定公开 release 审计进一步从核心与补充包共享首选中按规范码去重，各取 256 个
高频两字、三字词，并在不同位置合成一枚邻键多按。两字组给出 255 次恢复，合成
目标首项/可见均为 254，另有 1 次两层目标码冲突、1 次两层一致但偏离合成目标；
三字组给出 256 次恢复，合成目标首项/可见均为 255，另有 1 次一致偏离。两组干净
码保护失败均为 0。偏离例来自同一观察串可删除两枚互为邻键的真实歧义，说明公开
词频共识不能证明具体哪一键是误触，也正是该栏保持第二项而不抢首选的实证。优化
为最多七次删除后的精确 trie 查询后，同机预热 45 个固定样本的 median 约
0.004 ms、p95 约 0.010 ms；这只用于防止实现退化，不是跨设备延迟保证。
