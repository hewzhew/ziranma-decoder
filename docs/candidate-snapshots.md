...
`--expected-sha256`。这两个步骤目前刻意分开：槽位命令仍只接受独立摘要，
不会自行发现签名或选择信任密钥。项目尚未发布真实发布公钥，也没有签名命令、
私钥存储、密钥轮换或吊销策略；测试只使用显然为合成数据的内存密钥字节，
仓库和运行时都不生成或保存私钥。

候选包签名只认证候选包材料，不是 Windows PE/Authenticode 签名，不能证明
TSF DLL 或工具 EXE 的发布者身份，也不改变当前“不注册、不安装”的 Alpha
边界。
...
三槽状态、新类工厂读取 `current`，以及独立公钥驱动的 Ed25519 只读验签已经
完成。正式分发仍有两道门：

1. 当前没有真实发布公钥、签名命令、私钥保管、轮换或吊销策略；验签与槽位
   写入也保持为两个显式步骤，不能把测试签名能力称为已建立发布体系；
