...
十个 EXE 的 SHA-256 写入一个规范清单；清单自身的 SHA-256 是包标识。工具包
...
包，再检查十种受管工具的运行路径；仍被进程使用的包跳过，无法检查进程路径、
...
`typing-practice.cmd` 和 `wish-ime.cmd` 通过固定白名单解析 current。还没有工具槽时，它们兼容回退
...
追随内容寻址目录；复制失败时 current 保持不变。启动器的 `wish`、`alias`、`mode` 和 `practice` 模式
重新验证槽、完整清单、每个 EXE 摘要和包内文件集合，再直接打开 GUI，因此不会
闪出批处理窗口；`update` 只打开仓库固定的 `update-ime.cmd update`，仍使用可见控制台。
它不接受任意程序或路径。`practice` 只启动当前包内的原生仅内存练习区；若回退到
尚未包含对应工具的旧包，会明确失败而不搜索其他路径。`mode` 只启动单实例通知区
伴侣，不创建启动项。回退只交换工具槽，稳定启动器保持新版并兼容旧的七、八、九工具
清单，已有桌面入口不会因清单升级失效。
...
| 原生打字练习实验室 | 关闭旧窗口后再次打开；它不依赖 TSF DLL 换代 |
| 输入模式通知区图标 | 用户工具刷新后可启动伴侣；准确的中/A发布仍需新版 TSF DLL |
