...
九个 EXE 的 SHA-256 写入一个规范清单；清单自身的 SHA-256 是包标识。工具包
...
包，再检查九种受管工具的运行路径；仍被进程使用的包跳过，无法检查进程路径、
...
追随内容寻址目录；复制失败时 current 保持不变。启动器的 `wish`、`alias` 和 `practice` 模式
...
尚未包含该工具的旧包，会明确失败而不搜索其他路径。回退只交换工具槽，稳定启动器
保持新版并兼容旧的七工具和八工具清单，已有桌面入口不会因清单升级失效。
