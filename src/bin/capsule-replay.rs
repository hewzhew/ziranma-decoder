...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    match parse_options(std::env::args().skip(1))? {
        ...
        Options::Inputs {
            ...
            let mut metadata_guard = SegmentMetadataGuard::new(
                personal_cache
                    || personal_pair_cache
                    || personal_pair_comparison
                    || personal_word_comparison
                ...
                || personal_left_context_comparison
            {
                ...
                if !history_inputs.is_empty() {
                    ...
                    visit_private_inputs(
                        ...
                        |capsule| {
                            if personal_left_context_comparison {
                                redact_private_analysis_error(
                                    history_report
                            ...
                            } else if personal_code_comparison {
                                redact_private_analysis_error(
                                    history_report.learn_capsule_for_personal_code_comparison(
                                        &decoder, &mut state, capsule,
                                    ),
                                )?;
                            } else {
                                redact_private_analysis_error(
                                    history_report.learn_capsule_for_personal_cache(
                                        &decoder, &mut state, capsule,
                                    ),
                                )?;
                            }
                ...
                let frozen_state = (personal_pair_comparison
                    || personal_word_comparison
                    || personal_code_comparison
                    || personal_left_context_comparison)
                    .then(|| state.fork_for_frozen_evaluation());
                visit_private_inputs(
                    &mut loader,
                    &mut metadata_guard,
                    &inputs,
                    ...
                    |capsule| {
                        if personal_left_context_comparison {
                            let frozen_state = frozen_state
                                .as_ref()
                                .expect("left-context comparison requires a frozen history state");
                            redact_private_analysis_error(
                                report.observe_capsule_with_personal_left_context_comparison(
                                    &decoder,
                                    frozen_state,
                                    &mut state,
                                    capsule,
                            ...
                            let frozen_state = frozen_state
                                .as_ref()
                                .expect("code comparison requires a frozen history state");
                            redact_private_analysis_error(
                                report.observe_capsule_with_personal_code_comparison(
                                    &decoder,
                                    frozen_state,
                                    &mut state,
                                    capsule,
                        ...
                        } else if personal_pair_comparison {
                            let frozen_state = frozen_state
                                .as_ref()
                                .expect("pair comparison requires a frozen history state");
                            redact_private_analysis_error(
                                report.observe_capsule_with_personal_pair_comparison(
                                    &decoder,
                                    frozen_state,
                                    &mut state,
                                    capsule,
                        ...
                        } else if let Some(frozen_state) = &frozen_state {
                            redact_private_analysis_error(
                                report.observe_capsule_with_personal_word_comparison(
                                    &decoder,
                                    frozen_state,
                                    &mut state,
                                    capsule,
                        ...
                        } else if personal_pair_cache {
                            if compact {
                                redact_private_analysis_error(
                                    report.observe_capsule_with_compact_personal_pair_cache(
                                        &decoder, &mut state, capsule,
                                    ),
                                )?;
                            } else {
                            ...
                            redact_private_analysis_error(
                                report.observe_capsule_with_compact_personal_cache(
                                    &decoder, &mut state, capsule,
                                ),
                            )?;
                        } else {
            ...
            if human_report {
                ...
                println!(
                    "{}",
                    report.personal_left_context_comparison_terminal_report()
                );
            } else if personal_code_comparison {
                println!("{}", report.personal_code_comparison_terminal_report());
            } else if personal_pair_comparison {
                println!("{}", report.personal_pair_comparison_terminal_report());
            } else if personal_word_comparison {
                println!("{}", report.personal_word_comparison_terminal_report());
            } else if compact {
...
fn parse_options(
    ...
    let mut personal_pair_comparison = false;
    ...
    while let Some(argument) = arguments.next() {
        match argument.as_str() {
            ...
            "--personal-pair-comparison" => {
                if personal_pair_comparison {
                    return Err("--personal-pair-comparison can be given only once".into());
                }
                personal_pair_comparison = true;
            }
            "--personal-word-comparison" => {
                if personal_word_comparison {
                    return Err("--personal-word-comparison can be given only once".into());
                }
                personal_word_comparison = true;
            }
            "--personal-code-comparison" => {
                if personal_code_comparison {
                    return Err("--personal-code-comparison can be given only once".into());
                }
                personal_code_comparison = true;
            }
            "--personal-left-context-comparison" => {
                if personal_left_context_comparison {
                    return Err("--personal-left-context-comparison can be given only once".into());
                }
    ...
    if personal_pair_comparison && window_gap_ms.is_none() {
        return Err("--personal-pair-comparison requires --window-gap-ms".into());
    }
    if personal_pair_comparison && history_inputs.is_empty() {
        return Err(
            "--personal-pair-comparison requires at least one --history-input or \
             --history-session"
                .into(),
        );
    }
    if personal_pair_comparison && !compact {
        return Err("--personal-pair-comparison requires --compact".into());
    }
    if personal_word_comparison && window_gap_ms.is_none() {
        return Err("--personal-word-comparison requires --window-gap-ms".into());
    }
    if personal_word_comparison && history_inputs.is_empty() {
        return Err(
            "--personal-word-comparison requires at least one --history-input or \
             --history-session"
                .into(),
        );
    }
    if personal_word_comparison && !compact {
        return Err("--personal-word-comparison requires --compact".into());
    }
    if personal_code_comparison && window_gap_ms.is_none() {
        return Err("--personal-code-comparison requires --window-gap-ms".into());
    }
    if personal_code_comparison && history_inputs.is_empty() {
        return Err(
            "--personal-code-comparison requires at least one --history-input or \
             --history-session"
                .into(),
        );
    }
    if personal_code_comparison && !compact {
        return Err("--personal-code-comparison requires --compact".into());
    }
    if personal_left_context_comparison && window_gap_ms.is_none() {
        return Err("--personal-left-context-comparison requires --window-gap-ms".into());
    }
    if personal_left_context_comparison && history_inputs.is_empty() {
        return Err(
            "--personal-left-context-comparison requires at least one --history-input or \
             --history-session"
                .into(),
        );
    }
    ...
    if usize::from(public_context)
        ...
        + usize::from(personal_pair_comparison)
...
fn print_usage() {
    ...
    eprintln!(
        "Usage (frozen/causal personal comparison): cargo run --bin capsule-replay -- \\\n         <--history-input <OLDER.zic|OLDER.zcs>|--history-session <OLDER_SESSION>> ... \\\n         <--input <NEWER.zic|NEWER.zcs>|--session <NEWER_SESSION>> ... \\\n         --window-gap-ms <POSITIVE_MS> --personal-word-comparison --compact"
    );
    eprintln!(
        "Usage (word/pair personal comparison): cargo run --bin capsule-replay -- \\\n         <--history-input <OLDER.zic|OLDER.zcs>|--history-session <OLDER_SESSION>> ... \\\n         <--input <NEWER.zic|NEWER.zcs>|--session <NEWER_SESSION>> ... \\\n         --window-gap-ms <POSITIVE_MS> --personal-pair-comparison --compact"
    );
    eprintln!(
        "Usage (exact-code isolation comparison): cargo run --bin capsule-replay -- \\\n         <--history-input <OLDER.zic|OLDER.zcs>|--history-session <OLDER_SESSION>> ... \\\n         <--input <NEWER.zic|NEWER.zcs>|--session <NEWER_SESSION>> ... \\\n         --window-gap-ms <POSITIVE_MS> --personal-code-comparison --compact"
    );
    eprintln!(
...
mod tests {
    ...
    fn cli_requires_repeated_explicit_inputs() {
        ...
        assert!(!personal_pair_comparison);
        ...
        assert!(personal_pair_cache);
        let comparison = parse_options(vec![
            "--history-session".to_owned(),
            "1000-1".to_owned(),
            "--session".to_owned(),
            "2000-2".to_owned(),
            "--window-gap-ms".to_owned(),
            "5000".to_owned(),
            "--personal-word-comparison".to_owned(),
            "--compact".to_owned(),
        ])
        .unwrap();
        let Options::Inputs {
        ...
        assert!(personal_word_comparison);
        assert!(compact);
        let pair_comparison = parse_options(vec![
            "--history-session".to_owned(),
            "1000-1".to_owned(),
            "--session".to_owned(),
            "2000-2".to_owned(),
            "--window-gap-ms".to_owned(),
            "5000".to_owned(),
            "--personal-pair-comparison".to_owned(),
            "--compact".to_owned(),
        ])
        .unwrap();
        let Options::Inputs {
        ...
        assert!(personal_pair_comparison);
        let code_comparison = parse_options(vec![
            "--history-session".to_owned(),
            "1000-1".to_owned(),
            "--session".to_owned(),
            "2000-2".to_owned(),
            "--window-gap-ms".to_owned(),
            "5000".to_owned(),
            "--personal-code-comparison".to_owned(),
            "--compact".to_owned(),
        ])
        .unwrap();
        let Options::Inputs {
        ...
        assert!(personal_code_comparison);
        let left_context_comparison = parse_options(vec![
            "--history-session".to_owned(),
            "1000-1".to_owned(),
            "--session".to_owned(),
            "2000-2".to_owned(),
            "--window-gap-ms".to_owned(),
            "5000".to_owned(),
            "--personal-left-context-comparison".to_owned(),
            "--compact".to_owned(),
        ])
        .unwrap();
        let Options::Inputs {
