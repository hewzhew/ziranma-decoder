...
use crate::{
    ...
    NativeShortExactAbstention, NativeTabAssemblyState, TranspositionCalibrationLabel,
...
pub const WISH_SCHEMA_V22: &str = "ziranma-wish-v22";
...
pub const CURRENT_WISH_SCHEMA_VERSION: u8 = 22;
...
const WISH_PLAINTEXT_MAGIC_V22: &[u8] = b"ziranma-wish-v22\0";
...
impl WishSnapshot {
    ...
    pub fn supports_precise_candidate_ranking_personalization(&self) -> bool {
    ...
    pub fn supports_personal_selection_confirmation(&self) -> bool {
        self.source_schema_version >= 19
    }

    pub fn supports_short_word_extra_key_timing(&self) -> bool {
    ...
    pub fn supports_candidate_preference_labels(&self) -> bool {
        self.source_schema_version >= 22
    }
    ...
    fn render_with_event_version(&self, event_version: u8) -> Result<Vec<u8>, WishFeedbackError> {
        ...
        output.extend_from_slice(WISH_PLAINTEXT_MAGIC_V22);
    ...
    fn parse(input: &[u8]) -> Result<Self, WishFeedbackError> {
        ...
        let version = if input.starts_with(WISH_PLAINTEXT_MAGIC_V22) {
            reader.expect(WISH_PLAINTEXT_MAGIC_V22)?;
            22
        } else if input.starts_with(WISH_PLAINTEXT_MAGIC_V21) {
            ...
            reader.expect(WISH_PLAINTEXT_MAGIC_V19)?;
            19
        } else if input.starts_with(WISH_PLAINTEXT_MAGIC_V18) {
    ...
    match event {
        ...
        NativeFeedbackEvent::CandidatesPresentedWithProvenance {
            ...
            for provenance in provenance {
                ...
                if version >= 14 {
                    output.push(provenance.ranking_personalization().bits());
        ...
        NativeFeedbackEvent::CandidatePreferenceLabeled { code, text, action } => {
            if version < 22 {
                return Err(WishFeedbackError::InvalidSnapshot);
            }
            output.push(17);
            put_string(output, code)?;
            put_string(output, text)?;
            output.push(candidate_preference_action_tag(*action));
        }
        NativeFeedbackEvent::CandidatePopupTiming {
            first_frame_ms,
            fully_visible_ms,
        ...
        NativeFeedbackEvent::PersonalSelectionConfirmed {
            ...
            put_string(output, code)?;
            put_string(output, text)?;
            output.push(u8::from(*persistent_preferred));
            output.push(u8::from(*session_retained));
        }
        NativeFeedbackEvent::ShortWordExtraKeyTiming {
            observed_code,
            intended_code,
            ...
            output.push(16);
            put_string(output, observed_code)?;
            put_string(output, intended_code)?;
            put_usize(output, *extra_index)?;
            put_usize(output, inter_key_gaps_ms.len())?;
            for gap_ms in inter_key_gaps_ms {
                put_u32(output, *gap_ms);
            }
            put_usize(output, *visible_rank)?;
        }
    }
...
fn parse_event(
    ...
    let event = match reader.byte()? {
        ...
        15 if version >= 19 => NativeFeedbackEvent::PersonalSelectionConfirmed {
            code: reader.string()?,
            text: reader.string()?,
            persistent_preferred: reader.boolean()?,
            session_retained: reader.boolean()?,
        },
        16 if version >= 20 => {
            let observed_code = reader.string()?;
            let intended_code = reader.string()?;
            let extra_index = reader.usize()?;
            let gap_count = reader.usize()?;
            if gap_count > 6 {
                return Err(WishFeedbackError::InvalidSnapshot);
            }
            let mut inter_key_gaps_ms = Vec::with_capacity(gap_count);
            for _ in 0..gap_count {
                inter_key_gaps_ms.push(reader.u32()?);
            }
            NativeFeedbackEvent::ShortWordExtraKeyTiming {
                observed_code,
                intended_code,
        ...
        17 if version >= 22 => NativeFeedbackEvent::CandidatePreferenceLabeled {
            code: reader.string()?,
            text: reader.string()?,
            action: parse_candidate_preference_action(reader.byte()?)?,
        },
            ...
            for _ in 0..provenance_count {
                ...
                let item = if tag == 12 {
                    let personalization = NativeCandidatePersonalization::from_bits(reader.byte()?)
                        .ok_or(WishFeedbackError::InvalidSnapshot)?;
                    let ranking_personalization = if version >= 14 {
                        NativeCandidatePersonalization::from_bits(reader.byte()?)
                            .ok_or(WishFeedbackError::InvalidSnapshot)?
                    } else {
...
fn candidate_preference_action_tag(value: NativeCandidatePreferenceAction) -> u8 {
    match value {
        NativeCandidatePreferenceAction::Prefer => 1,
        NativeCandidatePreferenceAction::DeferToPublic => 2,
        NativeCandidatePreferenceAction::RestorePersonalization => 3,
    }
}

fn parse_candidate_preference_action(
    value: u8,
) -> Result<NativeCandidatePreferenceAction, WishFeedbackError> {
    match value {
        1 => Ok(NativeCandidatePreferenceAction::Prefer),
        2 => Ok(NativeCandidatePreferenceAction::DeferToPublic),
        3 => Ok(NativeCandidatePreferenceAction::RestorePersonalization),
        _ => Err(WishFeedbackError::InvalidSnapshot),
    }
}
...
mod tests {
    ...
    fn render_current_body_with_magic(snapshot: &WishSnapshot, magic: &[u8]) -> Vec<u8> {
        let event_version = if magic == WISH_PLAINTEXT_MAGIC_V8 {
            ...
            17
        } else if magic == WISH_PLAINTEXT_MAGIC_V18 {
            18
        } else if magic == WISH_PLAINTEXT_MAGIC_V19 {
            19
        } else if magic == WISH_PLAINTEXT_MAGIC_V20 {
            20
        } else if magic == WISH_PLAINTEXT_MAGIC_V21 {
            21
        } else {
            22
        };
        ...
        rendered.extend_from_slice(&current[WISH_PLAINTEXT_MAGIC_V22.len()..]);
        rendered
    }
    ...
    fn private_snapshot_round_trips_without_debug_surface() {
        ...
        assert!(rendered.starts_with(WISH_PLAINTEXT_MAGIC_V22));
        ...
        assert_eq!(parsed.source_schema_version(), 22);
        ...
        assert!(parsed.supports_candidate_preference_labels());
    ...
    fn v11_round_trips_stacked_candidate_personalization() {
        ...
        assert!(provenance[0].ranking_personalization().is_empty());
    }

    #[test]
    fn v14_round_trips_personal_evidence_and_actual_ranking_causes_strictly() {
        let mut snapshot = private_snapshot();
        let personalization = NativeCandidatePersonalization::PERSISTENT_EXACT
            .with(NativeCandidatePersonalization::SESSION_EXACT)
            .with(NativeCandidatePersonalization::LEFT_CONTEXT);
        let ranking_personalization = NativeCandidatePersonalization::LEFT_CONTEXT;
        snapshot.events[0].event = NativeFeedbackEvent::CandidatesPresentedWithProvenance {
            code: "ab".to_owned(),
            view: NativeCandidateView::Ordinary,
            page_start: 0,
        ...
        assert_eq!(provenance[0].personalization(), personalization);
        assert_eq!(
            provenance[0].ranking_personalization(),
            ranking_personalization
        );
        assert!(snapshot.render_with_event_version(13).is_err());

        let encoded = [
            candidate_source_tag(NativeCandidateSource::CoreExact),
            personalization.bits(),
            ranking_personalization.bits(),
        ];
        let ranking_offset = rendered
            .windows(encoded.len())
            .position(|window| window == encoded)
            .map(|offset| offset + 2)
            .unwrap();
        let mut unknown_ranking = rendered.clone();
        unknown_ranking[ranking_offset] = 1 << 7;
        assert!(WishSnapshot::parse(&unknown_ranking).is_err());
        let mut ranking_without_evidence = rendered;
    ...
    fn v16_round_trips_bounded_personal_phrase_adjacency_strictly() {
        ...
        falsely_downgraded.extend_from_slice(&rendered[WISH_PLAINTEXT_MAGIC_V22.len()..]);
    ...
    #[test]
    fn v19_round_trips_personal_selection_confirmation_outcomes_strictly() {
        let mut snapshot = private_snapshot();
        snapshot.events.push(WishEvent {
            milliseconds_before_marker: 0,
            event: NativeFeedbackEvent::PersonalSelectionConfirmed {
                code: "ab".to_owned(),
                text: "乙".to_owned(),
        ...
        let rendered = render_current_body_with_magic(&snapshot, WISH_PLAINTEXT_MAGIC_V19);
        assert!(rendered.starts_with(WISH_PLAINTEXT_MAGIC_V19));
        let parsed = WishSnapshot::parse(&rendered).unwrap();
        assert_eq!(parsed.source_schema_version(), 19);
        assert!(parsed.supports_personal_selection_confirmation());
        assert!(!parsed.supports_short_word_extra_key_timing());
        assert!(matches!(
            parsed.events().last().map(WishEvent::event),
        ...
        assert!(snapshot.render_with_event_version(18).is_err());

        let legacy = render_current_body_with_magic(&private_snapshot(), WISH_PLAINTEXT_MAGIC_V18);
        let parsed_legacy = WishSnapshot::parse(&legacy).unwrap();
        assert!(!parsed_legacy.supports_personal_selection_confirmation());
    }

    #[test]
    fn v20_round_trips_short_word_extra_key_timing_and_v19_rejects_it() {
        let mut snapshot = private_snapshot();
        snapshot.events.push(WishEvent {
            milliseconds_before_marker: 0,
            event: NativeFeedbackEvent::ShortWordExtraKeyTiming {
        ...
        assert!(snapshot.render_with_event_version(19).is_err());
    ...
    fn v21_round_trips_short_exact_abstention_and_v20_keeps_it_unrecorded() {
        ...
        let rendered = render_current_body_with_magic(&snapshot, WISH_PLAINTEXT_MAGIC_V21);
    ...
    #[test]
    fn v22_round_trips_candidate_preference_labels_strictly() {
        for action in [
            ...
            snapshot.events.push(WishEvent {
                milliseconds_before_marker: 0,
                event: NativeFeedbackEvent::CandidatePreferenceLabeled {
                    code: "jdj".to_owned(),
                    text: "讲讲".to_owned(),
                    action,
                },
            ...
            assert!(matches!(
                parsed.events().last().map(WishEvent::event),
                Some(NativeFeedbackEvent::CandidatePreferenceLabeled {
                    code,
                    text,
                    action: parsed_action,
                }) if code == "jdj" && text == "讲讲" && *parsed_action == action
            ));
            assert!(snapshot.render_with_event_version(21).is_err());
        ...
        let legacy = render_current_body_with_magic(&private_snapshot(), WISH_PLAINTEXT_MAGIC_V21);
        let parsed_legacy = WishSnapshot::parse(&legacy).unwrap();
        assert!(!parsed_legacy.supports_candidate_preference_labels());
    }
    ...
    fn v13_round_trips_explicit_public_candidate_order_policy_strictly() {
        for policy in [
            ...
            let rendered = render_current_body_with_magic(&snapshot, WISH_PLAINTEXT_MAGIC_V13);
            let parsed = WishSnapshot::parse(&rendered).unwrap();
